...
export default function Login({ dispatch }) {
    ...
    return (
        <div className='login card'>
            ...
            <div className='input-group'>
                <input type={showPassword ? 'text' : 'password'} placeholder='Password' name='password' onChange={(e) => handleCredetials(e)} value={credentials.password} />
                {validation.passwordErr && <p className='error-message'>{validation.passwordErr}</p>}
            </div>
            <div className='show-password'>
                <input type="checkbox" onChange={showPasswordClick}/>
                <p style={{marginLeft:6}}>Show Password</p>
            </div>
